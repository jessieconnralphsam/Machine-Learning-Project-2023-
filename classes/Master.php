	...
	function save_image(){
		...
		if(isset($_FILES['images']['tmp_name']) && count($_FILES['images']['tmp_name'])>0){
			...
			if(!empty($data)){
				// echo '<script>
				// 	// Define your data
				// 	var imageData = ' . json_encode($data) . ';

				// 	// Load the TensorFlow.js model
				// 	tf.loadLayersModel("path_to_your_model/model.json")
				// 		.then(function(model) {
				// 			// Preprocess your data if necessary
				// 			var preprocessedData = preprocessData(imageData);

				// 			// Convert the preprocessed data to a TensorFlow tensor
				// 			var tensorData = tf.tensor(preprocessedData);

				// 			// Make predictions using the loaded model
				// 			var predictions = model.predict(tensorData);

				// 			// Process the predictions as needed
				// 			handlePredictions(predictions);

				// 			// Get the output value from predictions
				// 			var outputFromModel = predictions[0]; // Assuming the output is in the first position of predictions array

				// 			// Pass the output value to PHP
				// 			window.location.href = "your_php_script.php?output=" + outputFromModel;
				// 		});
				// </script>';

				
				// // Code to make predictions and obtain the output value
				// $predictedOutput = "1234"; // Replace with the actual predicted output value

				// // Add the predicted output to the $data array or string
				// $data['predicted_output'] = $predictedOutput;
				// // If $data is a string, you can concatenate the predicted output value
				// // $data .= ", '{$predictedOutput}'";

				// // Insert the data into the database, including the predicted output
				// $sql = "INSERT INTO `images` (`album_id`, `original_name`, `path_name`, `user_id`, `predicted_output`) VALUES {$data}";
				// $save = $this->conn->query($sql);
				...
				// your_php_script.php
				// $outputFromModel = $_GET['output'];

				// // Query the database using the output value
				// $sql = "SELECT * FROM `your_table` WHERE `unique_column` = '{$outputFromModel}'";
				// $result = $this->conn->query($sql);

				// // Rest of the code to process the database result and handle retrieved values
